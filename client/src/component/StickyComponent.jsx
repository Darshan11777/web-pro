...
const ShrinkingHeader = ({ ComponentOne, ComponentTwo }) => {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start center", "end start"], // Control scroll trigger purely by ComponentOne
  });

  // Scale down and fade out transformations (locked to the progress of ComponentOne)
  const scale = useTransform(scrollYProgress, [0.1384143670608848, 0.4], [1, 0.4]);
  const opacity = useTransform(scrollYProgress, [0.1384143670608848, 0.3], [1, 0]);
  ...
  useEffect(() => {
    scrollYProgress.onChange((latest) => {
      const newPosition = latest < 0.1384143670608848 ? "relative" : "sticky";
  ...
  <div className="absolute top-0 right-0 h-[500px] w-[60%] transform "> {/* Adjust the positioning */}
    <img src='https://res.cloudinary.com/dbuuc0cdy/image/upload/v1730878950/Ellipse_56_xqgkks.png' alt="" className="h-full w-full object-cover" />
      ...
      <div ref={ref} className="relative   ">
        ...
        <motion.div
          className="top-0 z-[-1] pt-[70px] pb-[70px] " // Sticky only after it hits the top
          style={{
            scale,
            opacity,
