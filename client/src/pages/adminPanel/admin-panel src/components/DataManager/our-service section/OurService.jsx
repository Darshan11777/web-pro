...
const OurService = () => {
  ...
  const startEditing = (slide) => {
    navigate(`/admin/slides/our-service/${slide.id}/edit`);
  };

  const addNewSlide = () => {
    navigate("/admin/slides/our-service/new");
  };
  ...
  const highlightText = (text) => {
    ...
    return text.split(regex).map((part, index) =>
      part.toLowerCase() === searchTerm.toLowerCase() ? (
        <span key={index} className="font-bold text-indigo-600">{part}</span>
      ) : part
    );
  };
        ...
        <tbody>
          {currentSlides.map((slide) => (
            <tr key={slide.id} className="hover:bg-gray-100">
              <td className="p-4">
                <img src={slide.imgUrl} alt={slide.title} className="w-32 h-32 object-cover rounded" />
              </td>
              ...
              <td className="p-4">
                <div className="flex  flex-wrap gap-2">
                  {slide.tags.split(",").slice(0, maxTagsToShow).map((tag, index) => (
                    ...
                    <span key={index}  className="inline-block bg-success text-success bg-opacity-10 rounded-full px-3 py-1  text-sm font-semibold">
              ...
              <td className="p-4">
                <button className="hover:text-primary pr-3" onClick={() => startEditing(slide)}>
