...
import SlidesForm from "./OurWorkForm";
...
const OurWork = () => {
  ...
  const startEditing = (slide) => {
    navigate(`/admin/slides/our-work/${slide.id}/edit`); // Navigate to edit page
  };
  const addNewSlide = () => {
    navigate("/admin/slides/our-work/new"); // Navigate to add new slide page
  };
        ...
        <div className="grid h-fit grid-cols-1 grid-auto-rows: min-content md:grid-cols-2 lg:grid-cols-3 gap-6">
          {slides.map((slide, i) => (
            <div
              ...
              -hidden transform transition duration-300 ease-in-out"
            >
              <img
                src={slide.imgUrl}
                alt={slide.title}
