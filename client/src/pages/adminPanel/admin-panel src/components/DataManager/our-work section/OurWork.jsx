...
const OurWork = () => {
  ...
  const startEditing = (slide) => {
    navigate(`/admin/slides/our-work/${slide.id}/edit`);
  };

  const addNewSlide = () => {
    navigate("/admin/slides/our-work/new");
  };
  ...
  return (
    <div className="container mx-auto mt-10">
      <Breadcrumb pageName="Manage Our Work section Slides" />
        ...
        <tbody>
          {currentSlides.map((item) => (
            <tr key={item.id} className="hover:bg-gray-100">
              <td className="p-4">
                <img src={item.imgUrl} alt={`Image ${item.id}`} className="w-32 h-32 object-cover rounded" />
              </td>
              <td className="p-4 max-w-xs overflow-hidden text-ellipsis whitespace-wrap" > 
              ...
              <td className="p-4">
                <div className="flex flex-wrap gap-2">
                  {item.tags.split(",").map((tag) => (
                        <span key={tag} className="inline-block bg-success text-success bg-opacity-10 rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2">
              ...
              <td className="p-4">
                <button className="hover:text-primary pr-3" onClick={() => startEditing(item)}>
                  {/* Edit Icon */}
