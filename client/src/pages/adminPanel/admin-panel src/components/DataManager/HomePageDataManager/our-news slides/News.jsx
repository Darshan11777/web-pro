...
const News = () => {
  ...
  const indexOfLastItem = (currentPage + 1) * itemsPerPage; // Adjust for 0-based indexing
  console.log("indexOfLastItem", indexOfLastItem);
  const indexOfFirstItem = currentPage * itemsPerPage;
  console.log(" indexOfFirstItem", indexOfFirstItem);
  const currentSlides = filteredSlides.slice(indexOfFirstItem, indexOfLastItem);
  console.log("currentSlides", currentSlides);
  // const currentSlides = filteredSlides.slice(1, 2);
      ...
      <table className="min-w-full bg-white border border-gray-300 shadow-md rounded-lg overflow-hidden">
        <thead>
          <tr className="bg-gray-200 shadow-2 ">
            ...
            <th className="p-4 text-left w-1/2 max-w-xs overflow-hidden text-ellipsis whitespace-nowrap">
              Description
            </th>
        ...
        <tbody>
          {currentSlides.map((item, index) => (
            <tr key={item.id} className="hover:bg-gray-100">
              ...
              <td
                className="p-4 max-w-xs overflow-hidden text-ellipsis whitespace-nowrap"
              ...
              <td className="p-4">
                <div className="flex flex-wrap gap-2 ">
                  {item.tags.split(",").map((tag) => {
                    const highlightedTag = highlightSearchTerm(tag.trim()); // Assuming you have a highlightSearchTerm function
                    return (
                      <span
                        key={tag}
                        className="bg-success text-success  font-semibold inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium"
              ...
              <td className="p-4 ">
                ...
                {/* <DeleteIcon click={()=>deleteSlide(item.id)}/> */}
                <button
