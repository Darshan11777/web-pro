...
const ItServicesDetailsSlides = () => {
  ...
  const fetchSlides = async () => {
    const res = await axios.get(baseUrl + "our-services/our-services");
    setSlides(res.data);
  };

  const deleteSlide = async (id) => {
    if (window.confirm("Are you sure you want to delete this slide?")) {
      await axios.delete(baseUrl + `our-services/our-services/${id}`);
      fetchSlides();
    }
  };
                ...
                <div className="flex  flex-wrap gap-2">
                  {slide.tags
                    ...
                    .map((tag, index) => (
                      <span
                        key={index}
                        className="inline-block bg-success text-success bg-opacity-10 rounded-full px-3 py-1  text-sm font-semibold"
                      >
                        {highlightText(tag.trim())}
                      </span>
                    ))}
                  {slide.tags.split(",").length > maxTagsToShow && (
                    <span className="inline-block text-indigo-600 px-3 py-1 text-sm font-semibold">
                      ...
                    </span>
                  )}
                </div>
...
export default ItServicesDetailsSlides;
