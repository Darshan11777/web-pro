...
export const addSlide = (req, res) => {
  ...
  const query =
    "INSERT INTO our_service (title, description, tags, imgUrl) VALUES (?, ?, ?, ?)";
...
export const updateSlide = (req, res) => {
  const { title, description, tags, imgUrl } = req.body;
  const query =
    "UPDATE our_service SET title = ?, description = ?, tags = ?, imgUrl = ? WHERE id = ?";
  db.query(
    query,
...
export const addProcess = (req, res) => {
  const { title, description, tags, imgUrl } = req.body;
  const query =
    "INSERT INTO our_process (title, description, tags) VALUES (?, ?, ?)";
...
export const updateProcess = (req, res) => {
  ...
  const query =
    "UPDATE our_process SET title = ?, description = ?, tags = ? WHERE id = ?";
...
export const addWork = (req, res) => {
  ...
  const query =
    "INSERT INTO our_work (description, tags, imgUrl) VALUES (?, ?, ?)";
...
export const updateWork = (req, res) => {
  ...
  const query =
    "UPDATE our_work SET description = ?, tags = ?, imgUrl = ? WHERE id = ?";
  db.query(
    query,
...
export const addReview = (req, res) => {
  ...
  const query =
    "INSERT INTO reviews (name, review, email, rating, imgUrl) VALUES (?, ?, ?, ?, ?)";
...
export const updateReview = (req, res) => {
  const { name, review, email, rating, imgUrl } = req.body;
  const query =
    "UPDATE reviews SET name = ?, review = ?, email = ?, rating = ?, imgUrl = ? WHERE id = ?";
  db.query(
    query,
    [name, review, email, rating, imgUrl, req.params.id],
    (err, results) => {
      if (err) return res.status(500).json(err);
...
export const updateNews = (req, res) => {
  const { description, tags, imgUrl } = req.body;
  const query =
    "UPDATE news SET description = ?, tags = ?, imgUrl = ? WHERE id = ?";
  db.query(
    query,
    [description, tags, imgUrl, req.params.id],
    (err, results) => {
